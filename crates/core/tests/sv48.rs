...
fn satp_mode_warl_is_all_or_nothing() {
    ...
    // An unsupported MODE (10 = Sv57) is a COMPLETE no-op: MODE, ASID, and PPN all keep the old
    // value — not a MODE-only legalization. This is what Linux's set_satp_mode probing relies on.
    let before = c.read(SATP);
    let bad = (10u64 << 60) | (7 << 44) | 0x777;
    assert_eq!(
        wr_satp(&mut c, bad),
        before,
        "MODE=10 write is a total no-op (ASID/PPN unchanged too)"
    );
    // Every reserved MODE 1..=7 and 10..=15 is likewise a no-op.
    for m in (1..=7).chain(10..=15) {
        let before = c.read(SATP);
        assert_eq!(
            wr_satp(&mut c, (m << 60) | (3 << 44) | 0x333),
            before,
            "MODE={m} write is a no-op"
        );
    }
}
