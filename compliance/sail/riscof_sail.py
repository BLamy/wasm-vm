...
class sail(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # E1-T26: compile via the same-path Docker gcc (as spike does), but run the SAIL model
        # natively on the HOST — sail-riscv 0.12's prebuilt `sail_riscv_sim`. Unlike Spike, Sail
        # HONORS `hw_data_misaligned_support` (its default config lets misaligned scalar
        # load/stores proceed), so it is the reference that can validate our misaligned support.
        ...
        # Resolve the Sail simulator: $SAIL_SIM override, else the provisioned prebuilt tree.
        sail_env = os.environ.get('SAIL_SIM')
        if sail_env and os.path.exists(sail_env):
            self.sail_sim = sail_env
        else:
            import glob
            hits = sorted(glob.glob(os.path.join(self.repo_root, 'compliance', 'sail-riscv', '*', 'bin', 'sail_riscv_sim')))
            if not hits:
                print("Sail simulator not found — run 'bash compliance/provision.sh' (downloads sail-riscv), or set $SAIL_SIM.")
                raise SystemExit(1)
            self.sail_sim = hits[0]
        # Config override that makes Sail's misaligned atomics raise AlignmentException (cause
        # 4/6), matching our DUT (Sail's default is AccessFault for lrsc/amo). Scalar misaligned
        # already proceeds in the default config, matching us.
        self.sail_config = os.path.join(self.pluginpath, 'sail_config_override.json')
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       # runTests function
       self.compile_cmd = self.run_samepath+' riscv{1}-unknown-elf-gcc -march={0} \
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      if 'PMP' in ispec and 'pmp-grain' in ispec['PMP']:
...
#          cmd = self.compile_cmd.format(testentry['isa'].lower().replace('zicsr', ' ', 2), self.xlen, test, elf, compile_macros)
      ...
      # echo statement.
          if self.target_run:
            # Run the Sail model on the HOST over the Docker-compiled ELF. Sail's DEFAULT
            # config already lets misaligned scalar load/stores proceed (matching our DUT);
            # a `--config-override` is layered when present to align misaligned-atomic causes.
            override = ''
            if os.path.exists(self.sail_config):
                override = ' --config-override ' + self.sail_config
            simcmd = '{0}{1} --test-signature={2} --signature-granularity=4 {3}'.format(
                self.sail_sim, override, sig_file, elf)
          else:
            simcmd = 'echo "NO RUN"'
