...
class wasmvm(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # E1-T20: the Dockerized toolchain (gcc) is invoked via a same-path wrapper so RISCOF's
...
#          cmd = self.compile_cmd.format(testentry['isa'].lower().replace('zicsr', ' ', 2), self.xlen, test, elf, compile_macros)
      ...
      # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' run --signature={0} --signature-granularity=4 {1}'.format(sig_file, elf)
          else:
            simcmd = 'echo "NO RUN"'
