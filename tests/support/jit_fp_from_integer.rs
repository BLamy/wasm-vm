//! Actual generated FCVT.S.W/WU/L/LU modules, interpreter checks and literal goldens.
...
pub fn conversion(width: u32, rm: u32, rd: u32, rs1: u32) -> u32 {
    (0x68 << 25) | (width << 20) | (rs1 << 15) | (rm << 12) | (rd << 7) | 0x53
}
...
// Width selector, full x-register bits, [RNE, RTZ, RDN, RUP, RMM], new flags.
// These are literal powers, halfway integers, and integer-width boundaries.
const GOLDENS: &[(u32, u64, [u32; 5], u8)] = &[
    (0, 0x8000_0000_0000_0000, [0; 5], 0),
    (0, u64::MAX, [0xbf80_0000; 5], 0),
    (
        0,
        0xdead_beef_0100_0001,
        [
            0x4b80_0000,
            0x4b80_0000,
            0x4b80_0000,
    ...
    (0, 0x8000_0000, [0xcf00_0000; 5], 0),
    (1, 0xffff_ffff_0000_0000, [0; 5], 0),
    (
        1,
        u64::MAX,
        [
    ...
    (1, 1, [0x3f80_0000; 5], 0),
    (2, 0, [0; 5], 0),
    (2, u64::MAX, [0xbf80_0000; 5], 0),
    (2, 1 << 63, [0xdf00_0000; 5], 0),
    (
        2,
        (1 << 63) - 1,
        [
...
pub fn corpus(make: Factory) {
    ...
    for width in 0..4 {
        for rm in 0..8 {
            for (rs1, rd) in [(31, 0), (5, 31), (0, 5)] {
                ...
                for enabled in 0..=3 {
                    for mode in 0..8 {
                        for &(_, value, results, flags) in GOLDENS.iter().filter(|g| g.0 == width) {
                            let prior = (cases as u8).wrapping_mul(7) & 31;
                            seed(&mut m, enabled, mode, prior);
                            m.hart_mut().regs.write(rs1 as u8, value);
                            digest ^=
                                compare(e.as_mut(), &mut m, &b).rotate_left((cases % 64) as u32);
                            let resolved = if rm == 7 { u32::from(mode) } else { rm };
                            if enabled != 0 && resolved <= 4 {
                                let (bits, flags) = if rs1 == 0 {
                                    (0, 0)
                                } else {
                                    (results[resolved as usize], flags)
                                };
                                assert_eq!(
                                    m.hart().fregs.read_raw(rd as u8),
                                    boxed(bits),
                                    "literal width={width} value={value:016x} rm={rm} frm={mode} x{rs1}->f{rd}"
                                );
                                assert_eq!(
                                    m.hart().csr.fflags,
                                    prior | flags,
                                    "literal sticky flags"
                                );
                ...
                let mut random = 0x92ad_5761_4b30_e8cf_u64 ^ blocks;
                for n in 0..32 {
                    random ^= random << 13;
                    random ^= random >> 7;
                    random ^= random << 17;
                    seed(&mut m, 1 + n % 3, (n % 8) as u8, (n % 32) as u8);
                    m.hart_mut().regs.write(rs1 as u8, random);
    ...
    assert_eq!(cases, 21_504);
    eprintln!("FP_FROM_INT corpus cases={cases} register_flags_fnv={digest:016x}");
}
...
pub fn handoff_and_faults(make: Factory) {
    ...
    for width in 0..4 {
        ...
        let dynamic = block(at, &[conversion(width, 7, 0, 31)]);
        let fault = block(
            at + 0x100,
            &[conversion(width, 7, 5, 31), 0x0004_3283, 0x0013_8393],
        );
        let invalid = block(
            at + 0x200,
            &[conversion(width, 0, 31, 31), conversion(width, 6, 5, 31)],
        );
        let updated = block(at + 0x300, &[0x001f_8f93, conversion(width, 3, 31, 31)]);
        for b in [&dynamic, &fault, &invalid, &updated] {
            e.install(b);
            assert!(e.is_compiled(b.phys_start));
        }
        for phase in 0..16 {
            seed(&mut m, 2, 0, phase);
            m.hart_mut().regs.write(31, 16_777_217);
            compare(e.as_mut(), &mut m, &dynamic);
            assert_eq!(m.hart().fregs.read_raw(0), boxed(0x4b80_0000));
            assert_eq!(m.hart().csr.fflags, phase | 1);
            replace_fcsr(&mut m, 8 | (3 << 5));
            compare(e.as_mut(), &mut m, &dynamic);
            assert_eq!(m.hart().fregs.read_raw(0), boxed(0x4b80_0001));
            assert_eq!(m.hart().csr.fflags, 9);
            replace_fcsr(&mut m, 8 | (3 << 5));
            m.hart_mut().regs.write(31, 2);
            compare(e.as_mut(), &mut m, &dynamic);
            assert_eq!(m.hart().fregs.read_raw(0), boxed(0x4000_0000));
            assert_eq!(
                m.hart().csr.fflags,
                8,
            ...
            assert_eq!(m.hart().fregs.read_raw(5), boxed(0x4b80_0001));
            assert_eq!(m.hart().csr.fflags, 9);
            m.hart_mut().regs.pc = 0x4000_2000;
            compare(e.as_mut(), &mut m, &invalid);
            assert_eq!(m.hart().regs.pc, 0x4000_2004);
            m.hart_mut().regs.pc = 0x4000_2000;
            m.hart_mut().regs.write(31, 16_777_216);
            compare(e.as_mut(), &mut m, &updated);
            assert_eq!(m.hart().regs.read(31), 16_777_217);
            assert_eq!(m.hart().fregs.read_raw(31), boxed(0x4b80_0001));
            cases += 6;
        }
    }
    assert_eq!(cases, 384);
    eprintln!("FP_FROM_INT CSR/source-update/later-fault/invalid-prefix cases={cases}");
}
...
pub fn runloop(make: Factory) {
    ...
    let programs = [
        ...
        vec![0x0305, conversion(3, 7, 5, 31), 0x0010_0073],
    ...
    eprintln!("FP_FROM_INT runloop FS-off/invalid-rm prefix=1 bytes=2 later-fault-flags=3");
}
