...
PATCH_DIR="$ROOT/third_party/tailscale-connect/patches"
GVISOR_PATCH_DIR="$ROOT/third_party/tailscale-connect/gvisor-patches"
...
for patch in "$GVISOR_PATCH_DIR"/*.patch; do
  patch -d "$GVISOR_SOURCE" -p1 --forward --batch < "$patch"
done
(
  cd "$WORK/source"
  env \
