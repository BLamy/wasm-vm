...
async function runCandidate(candidate) {
  ...
  return {
    ...
    run: {
      command: "wasm-vm boot --net-slirp --virtio-rng --jit --jit-threshold 100 --interrupt-batching --append root=/dev/vda --no-reboot",
      exit,
      loginReached,
    ...
    commands: {
      ...
      simulate: `apk add --simulate --no-progress ${candidate.packages.join(" ")}`,
    ...
    simulate: extractCommand(stdout, "SIMULATE"),
...
printf 'E5T16D_''SIMULATE_BEGIN command=apk add --simulate --no-progress ${packages}\n'
apk add --simulate --no-progress ${packages} 2>&1
simulate_rc=$?
printf 'E5T16D_''SIMULATE_END rc=%s\n' "$simulate_rc"
[ "$simulate_rc" -eq 0 ] || all_ok=0
...
function extractArchitecture(stdout) {
  const body = extractBetween(stdout, "E5T16D_ARCH_BEGIN", "E5T16D_ARCH_END");
  return /\briscv64\b/u.test(body) ? "riscv64" : body.trim();
}

function extractRepositories(stdout) {
  const body = extractBetween(stdout, "E5T16D_REPOSITORIES_BEGIN", "E5T16D_REPOSITORIES_END");
  return [...new Set(body.match(/https:\/\/dl-cdn\.alpinelinux\.org\/alpine\/v3\.20\/(?:main|community)/gu) ?? [])];
}
