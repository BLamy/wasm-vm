...
const output = {
  ...
  repositories,
  policy: {
    install: "apk add --no-scripts --no-deps --no-progress <package>",
...
async function runCandidate(candidate) {
  ...
  return {
    ...
    commands: {
      ...
      add: `apk add --no-scripts --no-deps --no-progress ${candidate.packages.join(" ")}`,
...
printf 'E5T16D_''INSTALL_BEGIN command=apk add --no-scripts --no-deps --no-progress ${packages}\n'
apk add --no-scripts --no-deps --no-progress ${packages} 2>&1
install_rc=$?
