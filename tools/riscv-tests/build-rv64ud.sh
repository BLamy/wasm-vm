#!/usr/bin/env bash
# Build the official riscv-tests rv64ud-p (D-extension) suite (E1-T07) into committed ELFs.
#
# Runs INSIDE the E0-T13 reference toolchain:
#
#   tools/toolchain/run.sh -- tools/riscv-tests/build-rv64ud.sh
#
# Same pinned SHAs / reproducibility recipe as build.sh (E0-T19); only the source dir
# (isa/rv64ud) and -march (rv64ifd, so the assembler emits FP compute) differ. The p-env
...
obj="${work}/input.o"
count=0
for src in "${rt}"/isa/rv64ud/*.S; do
  name="$(basename "${src}" .S)"
  # -march=rv64ifd_zicsr: base integer + F+D (D ops under test) + zicsr (p-env CSR startup).
  cflags=(-march=rv64ifd_zicsr -mabi=lp64 -static -nostdlib -nostartfiles
    -fno-pic -ffreestanding -fno-common
    "-ffile-prefix-map=${rt}=." "-frandom-seed=rv64ud-${name}"
  ...
  riscv64-unknown-elf-gcc "${cflags[@]}" \
    ...
    -T "${rt}/env/p/link.ld" \
    "${obj}" -o "${out_dir}/rv64ud-p-${name}"
  count=$((count + 1))
done
rm -f "${obj}"

echo "riscv-tests: built ${count} rv64ud-p ELFs into ${out_dir}"
ls "${out_dir}" | grep rv64ud | sort | sed 's/^/  /'
