#!/usr/bin/env bash
# Build the official riscv-tests rv64uc-p (C-extension) suite (E1-T08) into committed ELFs.
#
# Runs INSIDE the E0-T13 reference toolchain:
#
#   tools/toolchain/run.sh -- tools/riscv-tests/build-rv64uc.sh
#
# Same pinned SHAs / reproducibility recipe as build.sh (E0-T19); only the source dir
# (isa/rv64uc) and -march (rv64gc, so the assembler emits FP compute) differ. The p-env
...
obj="${work}/input.o"
count=0
for src in "${rt}"/isa/rv64uc/*.S; do
  name="$(basename "${src}" .S)"
  # -march=rv64gc_zicsr: base integer + GC (compressed ops under test) + zicsr (p-env CSR startup).
  cflags=(-march=rv64gc_zicsr -mabi=lp64 -static -nostdlib -nostartfiles
    -fno-pic -ffreestanding -fno-common
    "-ffile-prefix-map=${rt}=." "-frandom-seed=rv64uc-${name}"
  ...
  riscv64-unknown-elf-gcc "${cflags[@]}" \
    ...
    -T "${rt}/env/p/link.ld" \
    "${obj}" -o "${out_dir}/rv64uc-p-${name}"
  count=$((count + 1))
done
rm -f "${obj}"

echo "riscv-tests: built ${count} rv64uc-p ELFs into ${out_dir}"
ls "${out_dir}" | grep rv64uc | sort | sed 's/^/  /'
