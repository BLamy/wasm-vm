#!/usr/bin/env bash
# Build the official riscv-tests rv64um-p (M-extension) suite (E1-T03) into committed ELFs.
#
# Runs INSIDE the E0-T13 reference toolchain:
#
#   tools/toolchain/run.sh -- tools/riscv-tests/build-rv64um.sh
#
# Same pinned SHAs / reproducibility recipe as build.sh (E0-T19); only the source dir
# (isa/rv64um) and -march (rv64im, so the assembler emits mul/div/rem) differ. The p-env
...
obj="${work}/input.o"
count=0
for src in "${rt}"/isa/rv64um/*.S; do
  name="$(basename "${src}" .S)"
  # -march=rv64im_zicsr: base integer + M (the ops under test) + zicsr (p-env CSR startup).
  cflags=(-march=rv64im_zicsr -mabi=lp64 -static -nostdlib -nostartfiles
    -fno-pic -ffreestanding -fno-common
    "-ffile-prefix-map=${rt}=." "-frandom-seed=rv64um-${name}"
  ...
  riscv64-unknown-elf-gcc "${cflags[@]}" \
    ...
    -T "${rt}/env/p/link.ld" \
    "${obj}" -o "${out_dir}/rv64um-p-${name}"
  count=$((count + 1))
done
rm -f "${obj}"

echo "riscv-tests: built ${count} rv64um-p ELFs into ${out_dir}"
ls "${out_dir}" | grep rv64um | sort | sed 's/^/  /'
