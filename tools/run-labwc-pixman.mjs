...
// E5-T16b: drive the real Alpine riscv64 labwc/pixman guest through the candidate-neutral T16a
...
const stdoutPath = path.join(evidenceDir, "labwc-console.log");
...
const guestScript = String.raw`m() {
  rss=0
  for p in $$ $(pidof labwc foot 2>/dev/null); do
  ...
  printf 'E5T16B_''METRIC phase=%s rss=%s interrupts=%s\n' "$1" "$rss" "$interrupts"
...
e5-t16b-start-labwc >/tmp/labwc.log 2>&1 &
labwc_job=$!
sleep 5
printf 'E5T16B_''LABWC_LOG_EARLY_BEGIN\n'
cat /tmp/labwc.log 2>/dev/null
printf 'E5T16B_''LABWC_LOG_EARLY_END\n'
if pidof labwc >/dev/null 2>&1; then printf 'E5T16B_''LABWC_STARTED=1\n'; else printf 'E5T16B_''LABWC_STARTED=0\n'; fi
e5-t16b-open-terminal >/tmp/foot.log 2>&1 &
foot_job=$!
sleep 5
printf 'E5T16B_''FOOT_LOG_EARLY_BEGIN\n'
cat /tmp/foot.log 2>/dev/null
printf 'E5T16B_''FOOT_LOG_EARLY_END\n'
if pidof foot >/dev/null 2>&1; then printf 'E5T16B_''FOOT_STARTED=1\n'; else printf 'E5T16B_''FOOT_STARTED=0\n'; fi
m OPEN_TERMINAL
sleep 5
m TYPE_100
sleep 2
...
sleep 2
if pidof foot >/dev/null 2>&1; then printf 'E5T16B_''APP_EXITED=0\n'; else printf 'E5T16B_''APP_EXITED=1\n'; fi
for p in $(pidof labwc 2>/dev/null); do kill "$p" 2>/dev/null; done
sleep 2
if pidof labwc >/dev/null 2>&1; then printf 'E5T16B_''WM_EXITED=0\n'; else printf 'E5T16B_''WM_EXITED=1\n'; fi
printf 'E5T16B_''LABWC_LOG_BEGIN\n'
cat /tmp/labwc.log 2>/dev/null
printf 'E5T16B_''LABWC_LOG_END\n'
printf 'E5T16B_''FOOT_LOG_BEGIN\n'
cat /tmp/foot.log 2>/dev/null
printf 'E5T16B_''FOOT_LOG_END\n'
m CLOSE
sleep 2
poweroff -f
`;
...
const child = spawn(cli, [
  ...
  env: {
    ...process.env,
    E5_T16B_TYPING_TEXT: plan.terminal.typingText,
    E5_T16B_TYPING_TEXT_SHA256: plan.terminal.typingTextSha256,
  },
...
assert.equal(timedOut, false, "labwc workload exceeded its bounded timeout");
assert.deepEqual(exit, { code: 0, signal: null }, `native labwc run failed: ${stderr.slice(-2_000)}`);
...
assert.match(stdout, /E5T16B_LABWC_STARTED=1/u, "labwc did not stay running after launch");
assert.match(stdout, /E5T16B_FOOT_STARTED=1/u, "foot did not stay running after launch");
assert.match(stdout, /E5T16B_APP_EXITED=1/u, "foot did not exit during close");
assert.match(stdout, /E5T16B_WM_EXITED=1/u, "labwc did not exit during close");
assert.match(stdout, /E5T16B_LAUNCH WLR_BACKENDS=drm WLR_RENDERER=pixman/u, "pixman/DRM launch proof is missing");
assert.doesNotMatch(stdout, /WLR_RENDERER=(?:gles2|gl2|vulkan)/u, "a GL renderer appeared in the finalist run");
assert.match(stdout, /E5T16B_LABWC_LOG_BEGIN/u);
assert.match(stdout, /E5T16B_FOOT_LOG_BEGIN/u);
assert.match(stdout, /reboot: Power down/u, "guest did not reach the kernel power-down line");

const captureMatch = stderr.match(/^E5T16B_CAPTURE_JSON (\{.*\})$/mu);
assert.ok(captureMatch, "native CLI did not emit a T16b capture");
...
await writeFile(runSummaryPath, `${JSON.stringify({
  task: "E5-T16b",
  command: "tools/run-labwc-pixman.mjs",
  image: result.image,
  ...
  renderer: { backend: "drm", renderer: "pixman", log: "E5T16B_LAUNCH" },
