...
out=${1:-"$repo/releases/wvseccomp-riscv64"}
target_dir=${CARGO_TARGET_DIR:-"$repo/target/wvseccomp-riscv64"}
mkdir -p "$(dirname -- "$out")"
...
cp "$target_dir/riscv64gc-unknown-linux-musl/release/wvseccomp" "$out"
chmod 0755 "$out"
