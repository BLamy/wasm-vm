#!/usr/bin/env node
// Actual built WasmMachine: compiled FP memory loop versus interpreter, in Chromium.
...
const out = path.resolve(process.argv[2] || "evidence/omarchy-profile/fp-memory-browser");
...
const words = [0x000020b7,0x30009073,0x0014d073,0x0023d073,
  ...
  load(false,0,5,0),store(true,0,5,16),load(true,31,5,8),store(false,31,5,24),
  load(true,5,5,16),store(true,5,5,32),addi(6,6,-1),bne(6,0,-28),
  integerLoad(3,7,16),integerLoad(6,8,24),integerLoad(3,9,32),
  csr(10,1),csr(11,2),csr(12,0x300),0x0000006f];
...
elf.writeBigUInt64LE(0x113355777fa12345n,0x3000);
elf.writeBigUInt64LE(0x01234567ffa67890n,0x3008);
await fs.writeFile(path.join(out,"fp-memory.elf"),elf);
...
try {
  ...
  for(const run of report.runs) {
    ...
    // registers() is [pc,x0..x31]; these are actual integer readbacks of
    // guest-written memory, including malformed-box FSW low bits.
    assert.equal(run.registers[8],"ffffffff7fa12345");
    assert.equal(run.registers[9],"ffa67890");
    assert.equal(run.registers[10],"ffffffff7fa12345");
    assert.equal(run.registers[11],"9"); assert.equal(run.registers[12],"7");
    assert.equal(BigInt(`0x${run.registers[13]}`)&0x8000000000006000n,0x8000000000006000n);
  }
  ...
  assert.ok(BigInt(jit.jitStats.retiredViaJit)>=3000n,"the FP memory loop itself must compile, beyond the short final spin");
  ...
  const capability = page.locator(".cap", {hasText:"Floating-point memory transfers"});
  ...
  await page.locator("#rm-search").fill("E5.5-T03u");
  await page.locator(".rm-g-label").filter({hasText:"E5.5-T03u"}).click();
