  ...
  const calibration = await page.evaluate(() => {
    ...
    for (const row of rows) {
      ...
      if (last && row.y === last.bottom) {
        last.bottom++; last.left = Math.min(last.left, row.left); last.right = Math.max(last.right, row.right + 1);
      } else bands.push({ left: row.left, right: row.right + 1, top: row.y, bottom: row.y + 1 });
    }
    const candidates = bands.filter(b => b.right-b.left <= 256 && b.bottom-b.top <= 32).map(b => {
      const rgba = [];
      for (let y=b.top; y<b.bottom; y++) rgba.push(...data.subarray(4*(y*canvas.width+b.left),4*(y*canvas.width+b.right)));
      return { ...b, width:b.right-b.left, height:b.bottom-b.top, rgba };
    });
    return { width:canvas.width, height:canvas.height, candidates, nativePng:canvas.toDataURL("image/png") };
  });
  await mkdir(out, { recursive:true });
  const nativePng = Buffer.from(calibration.nativePng.split(",")[1], "base64");
  delete calibration.nativePng;
  await writeFile(path.join(out, "native-canvas.png"), nativePng);
  for (const candidate of calibration.candidates) candidate.rgbaSha256 = sha256(Buffer.from(candidate.rgba));
  const result = { acceptance:false, purpose:"Raster calibration only; no timed interaction or text identity inferred",
    head, binding, baselineRestore, baselineSnapshot:normalSnapshot,
    nativePngSha256:sha256(nativePng), calibration, errors:{browser:browserErrors,http:httpErrors} };
  await writeFile(path.join(out,"calibration.json"), JSON.stringify(result,jsonReplacer,2)+"\n");
  await writeFile(path.join(out,"server.log"),serverOutput);
  console.log(JSON.stringify({acceptance:false, head, nativePngSha256:result.nativePngSha256,
    candidates:calibration.candidates.map(({rgba,...metadata})=>metadata)},null,2));
  phaseProgress("calibration:native-canvas","done");
  }
} catch (error) {
