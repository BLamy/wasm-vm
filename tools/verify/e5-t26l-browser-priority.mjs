...
const out = path.resolve(process.env.E5_T26L_OUT || path.join(repo, `evidence/e5-t26l/browser-${head.slice(0, 8)}`));
await mkdir(path.dirname(out), { recursive: true });
await mkdir(out); // Refuse overwriting every attempt, including failed attempts.
const retained = await mkdtemp(path.join(os.tmpdir(), "e5-t26l-priority-"));
...
const common = {
  ...
  E5_T26F_DIAGNOSTIC_PROFILE: retained, E5_T26F_DIAGNOSTIC_PORT: "61634",
...
for (const file of [command, "tools/verify/e5-t26f-resident-proof.mjs", "tools/verify/e5-t26f-discovery-observation.mjs",
  "tools/verify/e5-t26l-browser-priority.mjs", "crates/core/src/compile_queue.rs", "crates/core/src/lib.rs"])
...
const observation = discoveryObservation(raw, reuse.code); // Reject non-cap failures; retain the original T0/end.
assert.equal(headNow(), head);
...
console.log(JSON.stringify({ elapsedMs: observation.elapsedMs, fTimingPassed: observation.fTimingPassed,
  fVerified: false, deltas: observation.deltas }));
