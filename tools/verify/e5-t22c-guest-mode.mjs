...
const imageDir=path.resolve(repo,process.env.E5_T22C_IMAGE_DIR||"target/e5-t22c/desktop-image-v3");
const chunks=path.resolve(repo,process.env.E5_T22C_CHUNKS||"target/e5-t22c/chunks/desktop-v3");
...
}:JSON.parse(await readFile(path.join(repo,"tools/image/e5-t22c-desktop-image.json")));
...
await verifyChunkStore(chunks,publication);
...
async function command(verb,tag="DISPLAY"){
  ...
  return until(async()=>{const s=await page.evaluate(n=>desktopResize.serial().slice(n),offset);return s.includes((tag==="DISPLAY"?"E5T22C_":"E5T18D_")+tag+"_END")?s:null;},verb,120000);
}
const timer=setInterval(async()=>{try{
  const progress=await page.evaluate(async()=>({state:window.desktopResize?.state(),scheduler:await window.desktopResize?.controller()?.schedulerStats(),jit:await window.desktopResize?.controller()?.jitStats(),serial:window.desktopResize?.serial().slice(-600)}));
  console.log(JSON.stringify({retired:progress.scheduler?.retiredInstructions,frames:progress.state?.frames,serial:progress.serial}));await writeFile(path.join(out,"progress.json"),JSON.stringify(progress,null,2)+"\n");
...
try{
  ...
  await until(()=>page.evaluate(()=>!desktopResize.state().presentation.sizeMismatch&&desktopResize.state().presentation.latest?.resourceWidth===901),"initial requested mode");
  ...
  await until(async()=>(await content()).visible,"actual foot content",120000);
  ...
  for(const [width,height] of [[803,603],[640,480],[1280,800],[2560,1600],[801,601],[802,601]]){
    ...
    const result={width,height,started,paint,elapsed,guest,gpu,observation,status,marker,screenshotSha256:sha(screenshot)};
  ...
  await page.evaluate(()=>{const el=document.getElementById("viewport");el.style.width="802px";el.style.height="601px";});
  await sleep(2500);
  ...
  await writeFile(path.join(out,"failure.json"),JSON.stringify({error:String(error),errors,results},null,2)+"\n");
