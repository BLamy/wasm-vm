...
const source = readFileSync(new URL("./e5-t26f-browser-buffered-proc.mjs", import.meta.url), "utf8");
...
async function exercise({ env = {}, exits = [[0, null], [1, null]], existing = false,
  ...
  const scope = {
    ...
    readFile: async p => {
      ...
      sourceReads++; return changedSource && sourceReads > 13 ? "changed" : "source";
    },
...
test("actual wrapper creates a new seal and only the unchanged default-policy reuse", async () => {
  const r = await exercise({ env: { PATH: "/bin", E5_T26F_BUFFERED_PROC_OUT: "/fresh", E5_T26L_CHECKPOINT: "/old",
  ...
  assert.equal(cold.E5_T26F_DIAGNOSTIC_PROFILE, "/tmp/e5-t26f-buffered-proc-unique");
  assert.equal(cold.E5_T26F_DIAGNOSTIC_PORT, "61636"); assert.equal(cold.E5_T26F_HEADED, "0");
  assert.equal(cold.E5_T26F_IMAGE, "target/e5-t26f/resident-image-buffered-proc-v1/alpine-rootfs.ext4");
  assert.equal(cold.E5_T26F_IMAGE_INFO, "target/e5-t26f/resident-image-buffered-proc-v1/desktop-info.json");
  assert.equal(cold.E5_T26F_DESKTOP_ASSET_DIR, "target/e5-t26f/chunks/resident-buffered-proc-v1");
  ...
  assert.equal(Object.keys(bindings).length, 13);
  for (const file of ["tools/guest/e5-t26f-resident-aplay.sh", "tools/verify/e5-t26f-resident-image.mjs",
