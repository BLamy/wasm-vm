#!/usr/bin/env node
// Actual built WasmMachine: compiled FP comparison loop versus interpreter, in Chromium.
...
const out = path.resolve(process.argv[2] || "evidence/omarchy-profile/fp-comparisons-browser");
...
const cmp = (kind,rd,rs1,rs2) => ((0x50<<25)|(rs2<<20)|(rs1<<15)|(kind<<12)|(rd<<7)|0x53)>>>0;
const words = [0x000020b7,0x30009073,0x0014d073,0x0023d073,
  0x00002297,addi(5,5,-16),addi(6,0,400),
  load(false,0,5,0),load(false,1,5,4),load(true,2,5,8),load(false,3,5,16),
  cmp(2,7,0,1),cmp(1,8,0,1),cmp(0,9,1,0),cmp(2,10,2,0),cmp(1,11,2,0),cmp(2,12,3,0),
  addi(6,6,-1),bne(6,0,-28),
  csr(13,1),csr(14,2),csr(15,0x300),store(true,2,5,24),store(true,3,5,32),
  integerLoad(3,16,24),integerLoad(3,17,32),0x0000006f];
...
elf.writeUInt32LE(0,0x3000); elf.writeUInt32LE(0x80000000,0x3004);
elf.writeBigUInt64LE(0x012345677f800001n,0x3008); elf.writeUInt32LE(0x7f800001,0x3010);
await fs.writeFile(path.join(out,"fp-comparisons.elf"),elf);
...
try {
  ...
  for(const run of report.runs) {
    ...
    // registers() is [pc,x0..x31]. The guest computes signed-zero and NaN
    // comparisons and reads its own flags/source-FPR spills back to X registers.
    for(const [register,value] of [[7,"1"],[8,"0"],[9,"1"],[10,"0"],[11,"0"],[12,"0"],
      [13,"19"],[14,"7"],[16,"12345677f800001"],[17,"ffffffff7f800001"]]) {
  ...
  assert.ok(BigInt(jit.jitStats.retiredViaJit)>=3000n,"the FP comparison loop itself must compile, beyond the short final spin");
  ...
  const capability = page.locator(".cap", {hasText:"Floating-point comparisons"});
  ...
  await page.locator("#rm-search").fill("E5.5-T03v");
  await page.locator(".rm-g-label").filter({hasText:"E5.5-T03v"}).click();
