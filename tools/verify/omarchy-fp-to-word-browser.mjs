#!/usr/bin/env node
// Actual built WasmMachine: compiled float-to-word loop versus interpreter, in Chromium.
...
const out = path.resolve(process.argv[2] || "evidence/omarchy-profile/fp-to-word-browser");
...
const words = [0x000020b7,0x30009073,0x00145073,0x0021d073,
  ...
  floatLoad(0,0),floatLoad(31,4),floatLoad(2,8),floatLoad(3,12),
  toWord(0,0,10,0),toWord(0,2,11,31),toWord(1,7,12,2),toWord(1,0,13,3),
  fromInteger(0,0,4,10),arithmetic(false,0,5,4,4),addi(14,10,7),addi(6,6,-1),bne(6,0,-32),
  csr(20,1),csr(21,2),csr(22,0x300),
  store(true,0,5,16),store(true,31,5,24),store(true,4,5,32),store(true,5,5,40),
  integerLoad(3,16,16),integerLoad(3,17,24),integerLoad(3,18,32),integerLoad(3,19,40),0x0000006f];
...
[0x3fc00000,0xbfc00000,0x4f7fffff,0x7fc00000].forEach((bits,index) => elf.writeUInt32LE(bits,0x3000+index*4));
await fs.writeFile(path.join(out,"fp-to-word.elf"),elf);
...
try {
  ...
  for(const run of report.runs) {
    ...
    // registers() is [pc,x0..x31]. Check actual signed/unsigned word results,
    // sign extension, immediate integer consumption, FPR spills and CSRs.
    for(const [register,value] of [[10,"2"],[11,"fffffffffffffffe"],[12,"ffffffffffffff00"],
      [13,"ffffffffffffffff"],[14,"9"],[16,"ffffffff3fc00000"],[17,"ffffffffbfc00000"],
      [18,"ffffffff40000000"],[19,"ffffffff40800000"],[20,"19"],[21,"3"]]) {
  ...
  assert.ok(BigInt(jit.jitStats.retiredViaJit)>=3000n,"the float-to-word loop itself must compile, beyond the short final spin");
  ...
  const capability = page.locator(".cap", {hasText:"Float-to-word conversions"});
  ...
  await page.locator("#rm-search").fill("E5.5-T03y");
  await page.locator(".rm-g-label").filter({hasText:"E5.5-T03y"}).click();
