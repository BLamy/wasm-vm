...
const out = path.resolve(process.env.E5_T26F_COMPILE_QUEUE_OUT || path.join(repo, `evidence/e5-t26f/compile-queue-${head.slice(0, 8)}`));
await mkdir(path.dirname(out), { recursive: true });
await mkdir(out); // Refuse overwriting every attempt, including failed attempts.
const retained = await mkdtemp(path.join(os.tmpdir(), "e5-t26f-compile-queue-"));
...
const common = {
  ...
  E5_T26F_DIAGNOSTIC_PROFILE: retained, E5_T26F_DIAGNOSTIC_PORT: "61635",
...
for (const file of [command, "tools/verify/e5-t26f-resident-proof.mjs", "tools/verify/e5-t26f-discovery-observation.mjs", "tools/verify/e5-t26f-compile-queue-observation.mjs",
  "tools/verify/e5-t26f-browser-compile-queue.mjs", "crates/core/src/compile_queue.rs", "crates/core/src/lib.rs", "crates/wasm/src/lib.rs"])
