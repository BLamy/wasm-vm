...
const source = readFileSync(new URL("./e5-t26l-browser-priority.mjs", import.meta.url), "utf8");
...
async function exercise({ env = {}, exits = [[0, null], [1, null]], existing = false,
  ...
  const scope = {
    ...
    readFile: async p => {
      ...
      sourceReads++; return changedSource && sourceReads > 6 ? "changed" : "source";
    },
...
test("actual wrapper creates a new seal and only the unchanged default-policy reuse", async () => {
  const r = await exercise({ env: { PATH: "/bin", E5_T26L_OUT: "/fresh", E5_T26L_CHECKPOINT: "/old",
  ...
  assert.equal(cold.E5_T26F_DIAGNOSTIC_PROFILE, "/tmp/e5-t26l-priority-unique");
  assert.equal(cold.E5_T26F_DIAGNOSTIC_PORT, "61634"); assert.equal(cold.E5_T26F_HEADED, "0");
