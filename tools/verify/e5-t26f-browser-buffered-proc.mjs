...
const out = path.resolve(process.env.E5_T26F_BUFFERED_PROC_OUT || path.join(repo, `evidence/e5-t26f/buffered-proc-${head.slice(0, 8)}`));
await mkdir(path.dirname(out), { recursive: true });
await mkdir(out); // Refuse overwriting every attempt, including failed attempts.
const retained = await mkdtemp(path.join(os.tmpdir(), "e5-t26f-buffered-proc-"));
...
const common = {
  ...
  E5_T26F_DIAGNOSTIC_PROFILE: retained, E5_T26F_DIAGNOSTIC_PORT: "61636",
  E5_T26F_TIMEOUT_MS: "1800000",
  E5_T26F_IMAGE: "target/e5-t26f/resident-image-buffered-proc-v1/alpine-rootfs.ext4",
  E5_T26F_IMAGE_INFO: "target/e5-t26f/resident-image-buffered-proc-v1/desktop-info.json",
  E5_T26F_DESKTOP_ASSET_DIR: "target/e5-t26f/chunks/resident-buffered-proc-v1",
};
...
for (const file of [command, "tools/verify/e5-t26f-resident-proof.mjs", "tools/verify/e5-t26f-discovery-observation.mjs", "tools/verify/e5-t26f-compile-queue-observation.mjs",
  "tools/verify/e5-t26f-browser-buffered-proc.mjs", "crates/core/src/compile_queue.rs", "crates/core/src/lib.rs", "crates/wasm/src/lib.rs",
  "tools/guest/e5-t26f-resident-aplay.sh", "tools/verify/e5-t26f-resident-image.mjs", "tools/chunk_image.py",
