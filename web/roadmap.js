...
export const ROADMAP = [
  ...
  },
  {
    ...
    caps: [
      ...
      { name: "Browser display hotplug bridge", status: "verified", evidence: "E5-T22a: fresh verifier held actual direct/worker mode requests, 2,006 EDID samples, atomic rejection and old-resource invariance. Guest compositor resize remains T22c-d. Try display-hotplug.html." },
      { name: "Monitor mode survives guest GPU reset", status: "verified", evidence: "E5-T22e: native and actual-Wasm guest resets preserve monitor EDID while clearing resources, IRQs and stale queues; independent reset/reconfiguration attack and built 126/0 demo verified." },
      { name: "Viewport/DPR resize and stale-frame letterboxing", status: "verified", evidence: "E5-T22b independently verified: DPR 1/1.5/2, both browser backends, coalesced matching-frame repaint. Diagnostic at display-resize.html; actual guest mode adoption remains T22c-d." },
      { name: "Guest desktop in-place mode changes", status: "in-progress", evidence: "E5-T22c: pinned Weston DRM/pixman adapter and independent Wayland/EDID observer. Real client survival and release-to-frame timing are under test at desktop-resize.html; no performance verdict yet." },
      { name: "Bounded S/U cached-code permission work", status: "verified", evidence: "E5-T22f independently verified: unchanged-PMP S/U transitions perform zero cached-code audits; M-mode, revision revocation, PTE/PMP isolation, native/actual-Wasm parity, seeded attacks, and sabotage checks held. E5-T22c timing remains separate." },
      { name: "Profile-gated browser-JIT entry timing", status: "verified", evidence: "E5-T22g independently verified: profiling-off compiled Workers retain structural entry counters with zero performance.now() reads, profiling toggles and executor replacement preserve the gate, Chromium/Firefox fixed-retirement state parity holds, zero-time/off-replacement attacks pass, and the unchanged v7 desktop remains unprofiled. E5-T22c's separate two-second gate remains blocked." },
      ...
      { name: "Omarchy full-screen desktop and fresh warm launch", status: "in-progress", evidence: "E5.5-T03c verifies coherent SDR snapshot rendering and host-side viewport fitting. E5.5-T03e adds a hash-checked persistent boot-file cache and accurate restore phases without saving guest session state. E5.5-T03a/T03d remain blocked on slow physical input; responsive desktop and guest modesets are not yet verified." },
      ...
      { name: "Live priority for pending JIT compilation", status: "verified", evidence: "E5-T26l: real cross-pump priority, stale-byte safety, fixed budgets and independent clone/sabotage verified; new browser screen still fails F at 4727.945 ms, so its two-second gate remains unchanged" },
