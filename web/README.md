...
`areweturboyet`-style riscv-tests board that runs every committed
`tests/riscv-tests-bin/*` ELF in the browser and marks each row passed or failed.
...
Then open **http://localhost:8080**. The compliance board runs the copied
`web/assets/riscv-tests/*` fixtures; the console panel can still run `hello.elf`, where
`Hello from RV64` renders in the terminal and the status line shows `exited code=0
retired=83`.
