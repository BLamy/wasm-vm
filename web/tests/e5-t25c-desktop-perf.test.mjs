...
test("histogram and p95 retain a bimodal 500 ms tail", () => {
  const samples = [
    ...Array.from({ length: 10 }, () => ({ latencyMs: 4 })),
    ...Array.from({ length: 80 }, () => ({ latencyMs: 16 })),
    ...Array.from({ length: 10 }, () => ({ latencyMs: 500 })),
  ];
...
test("100 ms present-delay calibration is explicit and bounded", () => {
  const baseline = { latencyP50Ms: 24 };
  const delayed = { latencyP50Ms: baseline.latencyP50Ms + PRESENT_DELAY_CALIBRATION_MS };
  ...
  assert.equal(calibratePresentDelay(baseline, { latencyP50Ms: 40 }).shiftHeld, false);
});
