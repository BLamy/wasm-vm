...
# Usage: bash evidence/e5-t26o/run-final-clone.sh EXACT_COMMIT
set -euo pipefail
...
clone_dir=$(mktemp -d /private/tmp/e5-t26o-final.XXXXXXXX)
echo "E5-T26o exact head: $expected"
echo "E5-T26o retained clone: $clone_dir/repo"
...
echo "Run make verify-E5-T26o with compiler/test overrides scrubbed."
set +e
env "${unset_args[@]}" PATH="$trusted_path" bash --noprofile --norc -c \
  'cd "$1" && make verify-E5-T26o' e5-t26o-final "$clone_dir/repo"
result=$?
set -e
