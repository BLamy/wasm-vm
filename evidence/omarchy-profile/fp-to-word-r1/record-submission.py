...
commands={
 ...
 'affected': [
  ...
  ('native-test-clippy',['cargo','clippy','-p','wasm-vm-jit-runtime','--test','fp_to_word','--test','fp_to_word_verifier','--test','fp_arithmetic_verifier','--test','fp_from_integer_verifier','--','-D','warnings']),
  ('wasm-test-clippy',['cargo','clippy','-p','wasm-vm-wasm','--target','wasm32-unknown-unknown','--test','jit_fp_to_word','--test','jit_fp_to_word_verifier','--','-D','warnings']),
  ('core-handoff',['cargo','test','-p','wasm-vm-core','--features','trace','--lib','jit::tests','--','--nocapture']),
  ('fp-regression',['cargo','test','-p','wasm-vm-jit-runtime','--test','fp_moves','--test','fp_memory','--test','fp_comparisons','--test','fp_arithmetic','--test','fp_arithmetic_verifier','--test','fp_from_integer','--test','fp_from_integer_verifier','--test','precise_traps','--','--nocapture']),
  ...
  ('acceptance',['make','verify-E5_5-T03y','FP_TO_WORD_OUT='+str(out/'browser')]),
 ],
}[phase]
