...
commands={
 ...
 'affected': [
  ...
  ('core-memory',['cargo','test','-p','wasm-vm-core','--features','trace','--test','misaligned_virtual_pages','--','--nocapture']),
  ('translator',['cargo','test','-p','wasm-vm-jit-translate','--test','inline_tlb','--test','batch','--test','differential','--','--nocapture']),
  ('runtime-memory',['cargo','test','-p','wasm-vm-jit-runtime','--test','fp_moves','--test','precise_traps','--test','batching','--test','chaining','--test','invalidation','--','--nocapture']),
  ...
  ('hazards',['bash','tools/ci/determinism-hazards.sh']),
  ('acceptance',['make','verify-E5_5-T03u','FP_MEMORY_OUT='+str(out/'acceptance-browser')]),
 ],
}[phase]
