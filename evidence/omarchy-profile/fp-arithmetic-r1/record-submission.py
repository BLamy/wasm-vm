...
commands={
 'ci': [('ci',['make','-k','ci'])],
 'repair': [
  ('wasm-native-lib-final',['cargo','test','-p','wasm-vm-wasm','--lib']),
  ('browser-final',['node','tools/verify/omarchy-fp-arithmetic-browser.mjs',str(out/'browser-final')]),
 ],
 'affected': [
  ('clippy',['cargo','clippy','-p','wasm-vm-core','-p','wasm-vm-jit-translate','-p','wasm-vm-jit-runtime','-p','wasm-vm-wasm','--lib','--','-D','warnings']),
  ('softfloat',['cargo','test','-p','wasm-vm-core','--test','softfloat','--test','fp_arithmetic_flags','--','--nocapture']),
  ('core-handoff',['cargo','test','-p','wasm-vm-core','--features','trace','--lib','jit::tests','--','--nocapture']),
  ('fp-regression',['cargo','test','-p','wasm-vm-jit-runtime','--test','fp_moves','--test','fp_memory','--test','fp_comparisons','--test','precise_traps','--','--nocapture']),
  ...
  ('acceptance',['make','verify-E5_5-T03w','FP_ARITHMETIC_OUT='+str(out/'acceptance-browser')]),
 ],
}[phase]
