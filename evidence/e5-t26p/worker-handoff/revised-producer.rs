...
fn report_scalar_stores() {
    ...
    for &(name, word, xseed, fseed, address, expected, initial_resv, final_resv) in cases {
        for record in [false, true] {
...
fn report_atomics() {
    ...
    for (name, word, width, old, rhs, new_value) in [
        ...
        run_direct_fixture_modes(
            ...
            |fixture| {
                ...
                fixture.hart.resv = Some((DATA, width));
                if width == 4 {
            ...
            |result| {
                expect_no_trap(result, name);
                assert_eq!(result.hart.regs.read(5), old);
                assert_eq!(result.hart.resv, None);
                let actual = if width == 4 {
                    result.bus.inner.load32(DATA).unwrap() as u64
                } else {
        ...
        run_machine_fixture(
            ...
            |machine, outcome, _| {
                assert_eq!(outcome, RunOutcome::MaxInstrs);
                assert_eq!(machine.hart().regs.read(5), old);
                assert_eq!(machine.hart().resv, None);
                let actual = if width == 4 {
                    machine.bus_mut().load32(DATA).unwrap() as u64
                } else {
