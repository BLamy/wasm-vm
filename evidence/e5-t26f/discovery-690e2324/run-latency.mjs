// Existing read-only latency probe, reusing this release's newly authenticated seal.
// Not acceptance: original deadline and all guest commands remain unchanged.
...
const config = { ...invocation.common, E5_T26F_DIAGNOSTIC: 'reuse', E5_T26F_DIAGNOSTIC_LATENCY: '1', E5_T26F_OUT: directory };
const sourceBindings = {};
for (const f of [invocation.command, 'tools/verify/e5-t26f-resident-proof.mjs',
  'evidence/e5-t26f/discovery-690e2324/run-latency.mjs']) {
...
const file = path.join(directory, result.code === 0 ? 'diagnostic-iteration.json' : 'failure-post-restore-interaction-checks.json');
