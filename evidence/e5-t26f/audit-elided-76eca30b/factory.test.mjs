...
test("generated source removes exactly the requested guest block and retains guards", () => {
  ...
  assert.equal(setup.stdout, "\x1b[2J\x1b[H\n\n\n\n\x1b[42me5t26f-audit-elided-ready\x1b[0m\n");
  ...
  const original = sources["tools/verify/e5-t26f-browser-roundtrip.mjs"].toString();
  assert.equal(extractUnique(generated, "function readTopmostDragTitlebar()", "function observedDragTranslation("),
    extractUnique(original, "function readTopmostDragTitlebar()", "function observedDragTranslation("));
