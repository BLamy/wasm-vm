...
current browser-facing Node workload evidence is the six-operation matrix in
`web/runtime-benchmarks.json`, generated by `web/bench-runtime-workloads.mjs`; it is separate from
this boot-time MIPS baseline. The E2-T23 timekeeping spec measured the *idle* browser ratio at ~0.05
