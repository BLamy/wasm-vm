...
| `--blk-log` (E2-T19) | traces every virtio-blk request to stderr as `blk: <op> sector=N len=M status=S` (`op` ∈ IN/OUT/FLUSH/GET_ID; `status` 0=OK/1=IOERR/2=UNSUPP). The go-to for fs stalls and corruption: a boot that hangs after `EXT4-fs … mounting` shows whether reads are being serviced (and their sectors); a `status=1` is a backend I/O error; the FLUSH count vs writes reveals barrier/journal ordering. Requires `--drive`. |
